...
class ExcelBookLoader:
	...
	def load_book(self):
		...
		try:
			...
			return []
		print_log(u"Load book {} total words {}.".format(self.book_path, len(all_words)))
		return all_words
...
class BookDatabase:
	...
	def load_book(self):
		...
		if not self.all_words:
			print_log(Fore.RED + u"There are no words in book {}.".format(self.cur_book))
			return
		self._choose_mode()
	...
	def get_pos_range(self):
		return self.cur_pos, len(self.word_list)
	...
	def word_display(self):
		...
		while item and not self.should_exit:
			...
			try:
				...
				print_log("Escape one word due to encoding error!")
				pass
			item = self.db.next_word()
...
def run_loop(change_book=False):
	...
	while True:
		...
		if c == 'e':
			...
			ctrl.db.mark_hard(True)
		elif c == 'd':
			ctrl.db.mark_hard(False)
		elif c == 'n':
			ctrl.show_cur = False
		elif c == 'b':
			ctrl.db.cur_pos -= 2
